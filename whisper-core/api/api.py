...
async def get_all_entries(limit: int = 100, offset: int = 0, count: bool = False,
    ...
    if feed_title:
      logger.info(f"---------------------{feed_title}")
    entries = db_service.get_entries(feed_title, search_value, limit, offset)
...
async def chat_with_llm(
    ...
    """与 LLM 模型进行对话"""
    model_name = "CloseAi/Claude-3-7-Sonnet"
    print(f"\n开始调用模型 {model_name}")
...
# 添加主题分析路由
app.include_router(theme_analysis_router, prefix="/theme", tags=["theme-analysis"])


from api.web_search_api import router as web_search_router
app.include_router(web_search_router, prefix="/web_search", tags=["web_search"])


from api.materials_article_api import router as materials_article_router
app.include_router(materials_article_router, prefix="/materials_article", tags=["materials_article"])
