...
class DeepResearchWorkflow:
    def __init__(self):
        ...
        self.initial_query_node = InitialQuerySubQueriesNode(tid=workflow_tid)
        self.search_report_node = SearchReportNode(tid=workflow_tid)
        self.per_analysis_report = PerAnalysisReportNode(tid=workflow_tid)
        # 添加节点到工作流
        self.workflow.add_node(self.initial_query_node)
        self.workflow.add_node(self.search_report_node)
        self.workflow.add_node(self.per_analysis_report)
        
        # 设置节点执行顺序 (即使只有一个节点，也最好设置)
        self.workflow.set_node_order([self.initial_query_node.name,
                                      self.search_report_node.name,
                                      self.per_analysis_report.name])
    ...
    async def astream_execute(self, question: str, initial_context: Dict[str, Any] = None) -> AsyncGenerator[Tuple[str, str], None]:
        ...
        workflow_initial_inputs = {"initial_ai_search_query": question}
        if initial_context:
