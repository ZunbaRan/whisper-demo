...
# **注意事项：**
    * 所有查询应该明确规定时间限制为“最近两周”
    * 对于所有“某公司，某人”等代词一律判定为非具体事件，因为无法去搜索和查证

# 输出要求:
请严格以JSON格式返回结果。该JSON对象应包含三个键：
1.  `found_concrete_events`: 一个对象列表，包含所有在本次分析 `preliminary_report_markdown` 过程中，经过步骤二判断后认为信息已相对充分、具有较好爆款潜力的【具体事件】的简洁描述。
    found_concrete_events 中的每个对象应包含以下字段：
        * `event_description`: 一个字符串，描述该具体事件的核心内容（一句话概括）。
        * `event_details`: 一个字符串，根据当前搜索信息中的具体细节，对该事件进行更详细的描述。
