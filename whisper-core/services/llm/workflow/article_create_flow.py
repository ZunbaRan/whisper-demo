...
class ArticleCreateFlow:
    ...
    def _get_tid_dir(self, feed_title:str, tid: str) -> Path:
        """获取任务ID对应的目录
        ...
        # 确保tid是字符串类型
        tid_str = str(tid)
        tid_dir = self._output_dir / feed_title / tid_str
    ...
    async def create_article(self, content: str, feed_title: str) -> AsyncGenerator[Tuple[str, str], None]:
        """创建文章
        ...
        tid_dir = self._get_tid_dir(feed_title, tid)
